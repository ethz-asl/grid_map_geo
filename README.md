...
across), see the companion `grid_map_geo_wavelet_quadtree` package, which
compresses a DSM into a multi-resolution wavelet quadtree store on disk and
loads a bounded working-set region from it instead of the whole raster at
once. See that package's own README for usage.
